...
for (j in c("ICL")){
  ...
  for (t in 1:length(at)) { 
    name <- paste0("hhwacc_gr_",at[t],"_",j,"_pct") 
...
```{r}
IOU_ICAaccess_grid <- 
  rbind(select(PGE_ICAaccess, IOU, tothh_Cpoly, GEOID, intersect(starts_with("hhwacc_gr_"), ends_with("_pct"))),
        select(SCE_ICAaccess, IOU, tothh_Cpoly, GEOID, intersect(starts_with("hhwacc_gr_"), ends_with("_pct")))) %>%
...
all <- IOU_hhwacc_grid %>% 
  ggplot(aes(x=threshold, y=hhwacc_pct, shape=DER)) + plotops_allgrid
all
...
```{r}
IOU_ICAaccess_grid <- 
  rbind(select(PGE_ICAaccess, IOU, tothh_Cpoly, GEOID, intersect(starts_with("hhwacc_ge_"), ends_with("_pct"))),
        select(SCE_ICAaccess, IOU, tothh_Cpoly, GEOID, intersect(starts_with("hhwacc_ge_"), ends_with("_pct")))) %>%
  melt( id=c("IOU", "tothh_Cpoly", "GEOID")) %>%
  mutate(hhwacc_Cpoly = value*tothh_Cpoly) %>%    # households per Cpoly, not percent
  group_by(IOU, GEOID, variable) %>%
  summarise(avgacc_bg_pct = mean(value, na.rm=TRUE), # avg percent access in bg
...
```{r}
IOU_ICAaccess_grid <- 
  rbind(select(SCE_ICAaccess, IOU, tothh_Cpoly, GEOID, intersect(starts_with("hhwacc"), ends_with("_pct"))),
        select(PGE_ICAaccess, IOU, tothh_Cpoly, GEOID, intersect(starts_with("hhwacc"), ends_with("_pct")))) %>%
  melt( id=c("IOU", "tothh_Cpoly", "GEOID")) %>%
  mutate(hhwacc_Cpoly = value*tothh_Cpoly) %>%    # households per Cpoly, not percent
  group_by(IOU, GEOID, variable) %>%
  summarise(avgacc_bg_pct = mean(value, na.rm=TRUE), # avg percent access in bg
