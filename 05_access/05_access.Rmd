...
This code calculates overall grid access for households in PG&E and SCE service territories, considering housing types.
...
plotops <- list(theme_light(), facet_grid(IOU~., scales="fixed"), 
  ...
  labs(x="Access threshold (kW)", y="Portion of households with access", color="Housing size"))
```
...
ggsave("figures/access/IOU_access_housing_size.png", a, width=8, height=5.5)
```
...
ggplot(compar_acc, aes(x = year, y = value, fill = threshold)) +
  ...
  scale_fill_manual(values = c("lightgrey", "#c9f0d3", "#77b1a9", "#4b5d67"),
                    labels = c(expression(italic("No access")), "10 kW", "1.5 kW", ">0 kW")) +
  labs(x = "", 
       y = expression("Households x10"^"6"),
       fill = "Access threshold") +
  theme_light()
