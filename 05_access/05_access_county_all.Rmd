...
IOU_access_county_plot <- ggplot(plot) +
  ...
  labs(
    ...
    color=expression("Total households served x10"^"3"),
    fill=expression("Total households served x10"^"3")
  ) + theme_light() +
  theme(axis.text.x = element_text(size = 9, angle = 45, hjust = 1),
        legend.margin=margin(), legend.text=element_text(size=10)) +
  scale_y_continuous(labels = scales::percent_format(scale = 100)) +
  scale_color_discrete_sequential(palette="Dark Mint", nmax=4, order=1:4) +
  scale_fill_discrete_sequential(palette="Dark Mint", nmax=4, order=1:4) +
  guides(shape=guide_legend(nrow=4,title.position="top"))

ggsave("figures/access/IOU_access_county.png", IOU_access_county_plot,  width = 13.5, height = 6.5, units = "in")
