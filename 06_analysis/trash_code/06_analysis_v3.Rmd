...
SCE_ICAalldemo_housing <- read.csv("SCE/circuitfiles/SCE_ICAalldemoalloc_housing.csv", header = TRUE, na.strings = "?", colClasses = as.character(SCEhousingtypes$x))

PGEhousingtypes <- read.csv("PGE/circuitfiles/PGE_ICAalldemo_housing_classes.csv", header = TRUE, na.strings = "?")
PGE_ICAalldemo_housing <- read.csv("PGE/circuitfiles/PGE_ICAalldemoalloc_housing.csv", header = TRUE, na.strings = "?", colClasses = as.character(PGEhousingtypes$x))

SCE_ICAalldemo <- merge(SCE_ICAalldemo, SCE_ICAalldemo_housing[, c("GEOID", "ICL_kWstr", "MUD_pct", "duplex_pct", "smallmulti_pct", "largemulti_pct")], by = "GEOID")
PGE_ICAalldemo <- merge(PGE_ICAalldemo, PGE_ICAalldemo_housing[, c("GEOID", "ICL_kWstr", "MUD_pct", "duplex_pct", "smallmulti_pct", "largemulti_pct")], by = "GEOID")
### ------------------------###
```
...
cols = c("GEOID","CircuitName","IOU","tothh_Cpoly",
         "ICL_e_kWphh_cap")

# cols_extra_housing = c("ICL_kWstr", "MUD_pct", "duplex_pct", "smallmulti_pct", "largemulti_pct")
```
...
#                                         "Population identifying as Latinx (%)"), f_cat=c("demo_race_","demo_race_","demo_race_"), span_SCE_ICL=c(NA,NA,NA), span_PGE_ICL=c(NA,NA,NA)))
# 
# featscales <- rbind(featscales,
...
#                                f_binwidth=c(0.05,0.05,0.05), f_div=c(0.01,0.01,0.01), y_liml=c(NA,NA,NA), y_limh=c(NA,NA,NA),
#                                f_xlab=c("Population identifying as Black (%)","Population identifying as Asian (%)", 
#                                         "Population identifying as Black (%)"), f_cat=c("demo_race_","demo_race_","demo_race_"), span_SCE_ICL=c(NA,NA,NA), span_PGE_ICL=c(NA,NA,NA)))
```
...
```{r}
for (i in 32:35){ #from 1 to 31: normal features // from 32 to 35: extra housing features
...
lowessdf <- function(i){ i=17
  ...
  df <- df %>% na.omit() %>% mutate(tothh_Cpoly_int = round(tothh_Cpoly*1000000,0)) %>% group_by(var_bin, IOU) %>%
    summarise(ICL = wtd.quantile(ICL_e_kWphh_cap, probs=0.5, weight=tothh_Cpoly_int, na.rm=TRUE, normwt=FALSE)[[1]],
              tothh_Cpoly = sum(tothh_Cpoly, na.rm=TRUE)) %>% as.data.frame() %>% 
    melt(id=c("var_bin","tothh_Cpoly","IOU")) %>% na.omit() %>%
    mutate(IOU = factor(IOU, levels=c("PGE","SCE")))
  return(df)
}
```
...
#MEDIAN CALCULATION - VERSION N°1
...
#            IOU = factor(IOU, levels=c("PGE","SCE"))) %>% select(-c("variable")) %>% dplyr::rename("wtdmed"="value")
# }
...
df_ft <- df_ft %>% merge(cidf, by=c("IOU","var_bin"), all.x=TRUE)
  ...
  write.csv(df_ft, file=paste0("figures/lowessci/",featscales[i,"f_cat"],featscales[i,"feature"],"_lfit_V2.csv"), row.names=FALSE)
}
rm(ci,cidf)
toc()
```  
...
ggsave("demo_race_all.png", a, path="figures/lowesscomb/", width=8, height=5.5)
...
print(b1)
ggsave("demo_inc_all.png", b1, path="figures/lowesscomb/", width=8, height=5.5)
...
ggsave("demo_inc_medhhinc_2.png", b2, path="figures/lowesscomb/", width=8, height=5.5)
```
...
ggsave("demo_ces_all_pollution.png", c1, path="figures/lowesscomb/", width=8, height=5.5)
...
ggsave("demo_ces_all_population.png", c2, path="figures/lowesscomb/", width=8, height=5.5)
```

Lowess plots - housing types

```{r}
df_p <- rbind(mutate(read.csv(paste0("figures/lowessci/",featscales[21,"f_cat"],featscales[21,"feature"],"_lfit.csv"), 
                                    header=T, na.strings="?", colClasses=c(rep("factor",1),rep("numeric",22))),
                           feature=featscales[21,"feature"]), 
              mutate(read.csv(paste0("figures/lowessci/",featscales[22,"f_cat"],featscales[22,"feature"],"_lfit.csv"), 
                                    header=T, na.strings="?", colClasses=c(rep("factor",1),rep("numeric",22))),
...
print(d1)
ggsave("demo_hs_all.png", d1, path="figures/lowesscomb/", width=8, height=5.5)
...
print(d2)
ggsave("demo_hs_medyrbuilt_2.png", d2, path="figures/lowesscomb/", width=8, height=5.5)
```

Combined Figure

```{r}
plot_grid(b1, b2, c1, c2, d1, d2, nrow=3, ncol=3, scale=0.95, labels=c('a','b','c','d','e','f'), label_size=12)

ggsave("figures/lowesscomb/demo_cesinchs.pdf", width=25, height=20)
...
plotops <- list(theme_light(), facet_grid(IOU~., scales="fixed"),
  ...
  scale_x_continuous(breaks=seq(0,100,by=20)), #scale_y_continuous(breaks=scales::pretty_breaks(3)),
  scale_color_discrete_qualitative(palette="Dark3"), scale_fill_discrete_qualitative(palette="Dark3"),
  labs(x="Type of housing (%)", y=plotlabs[1], fill="Features", color="Features"))

e <- ggplot(filter(df_p), aes(x=var_bin*100, weight=tothh_Cpoly)) + plotops +
   scale_y_continuous(breaks=seq(0,5,1)) + coord_cartesian(ylim=c(0,5))

ggsave("demo_housing_types.png", e, path="figures/lowesscomb/", width=8, height=5.5)
