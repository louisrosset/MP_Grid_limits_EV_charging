...
VERSION COUNTY - Computing access for all counties (top 30)
...
# PGE_counties <- c("Alameda", "San Francisco", "Santa Clara")
# SCE_counties <- c("Los Angeles","Orange","San Bernardino")

iou_counties <- c("PGE_Alameda", "PGE_Santa Clara", "PGE_San Francisco", "PGE_Contra Costa", 
                  "PGE_Fresno", "PGE_San Mateo", "PGE_San Joaquin", "PGE_Kern",
                  "PGE_Solano", "PGE_Sonoma", "PGE_Monterey", "PGE_Marin", 
                  "PGE_San Luis Obispo", "PGE_Yolo", "PGE_Butte", "PGE_Santa Cruz", 
                  "PGE_Placer", "PGE_Santa Barbara", "PGE_Humboldt", "PGE_Napa",
                  "PGE_Sutter", "SCE_Los Angeles", "SCE_Orange", "SCE_San Bernardino",
                  "SCE_Riverside", "SCE_Ventura", "SCE_Tulare", "SCE_Santa Barbara",
                  "SCE_Kern", "SCE_Kings")

#Plots for top 8 for each IOU
iou_counties_plot <- c("PGE_Alameda", "PGE_Santa Clara", "PGE_San Francisco", "PGE_Contra Costa", 
                  "PGE_Fresno", "PGE_San Mateo", "PGE_San Joaquin", "PGE_Kern",
                  "SCE_Los Angeles", "SCE_Orange", "SCE_San Bernardino", "SCE_Riverside",
                  "SCE_Ventura", "SCE_Tulare", "SCE_Santa Barbara", "SCE_Kern")
```
...
for (i in iou_counties) {
    column_name <- paste0("span_", i, "_ICL")
...
df <- df %>% 
  ...
  as.data.frame()
    ...
    for (ioucty in iou_counties) {
      iou = sapply(strsplit(ioucty, "_"), function(x) x[1])
      cty = sapply(strsplit(ioucty, "_"), function(x) x[2])
      
        df_subset <- df %>%
            subset(county == cty & IOU == iou)
        ...
        featscales[[paste0("span_", ioucty, "_ICL")]][i] = lobj$pars$span
    }
}

rm(lobj, df_subset)

span_col <- c(sapply(iou_counties, function(ioucty) paste0("span_", ioucty, "_ICL")))
...
df = rbind(select(PGE_ICAalldemo, c(cols, featscales[i,"feature"])), select(SCE_ICAalldemo, c(cols, featscales[i,"feature"])))
df <- df %>% filter(county %in% sapply(strsplit(iou_counties, "_"), function(x) x[2])) #CHANGED HERE
  df = df[which(!is.na(df[[featscales[i,"feature"]]])),]
  df$var_bin = as.numeric(as.character(cut(df[[featscales[i,"feature"]]], 
        breaks=seq(featscales[i,"f_min"], featscales[i,"f_max"], by=featscales[i,"f_binwidth"]), 
        labels=seq(featscales[i,"f_min"], featscales[i,"f_max"]-featscales[i,"f_binwidth"], by=featscales[i,"f_binwidth"]))))
...
df_ft <- df %>%
  group_by(var_bin, IOU, county) %>% #CHANGED HERE
  summarise(
...
for (ioucty in iou_counties) {
  ...
  df_n <- df[,c("IOU", "county", "var_bin",featscales[i,"feature"],"tothh_Cpoly","tothh_Cpoly_int",
              "ICL_e_kWphh_cap")] %>% filter(county==cty, IOU==iou)
...
loessbt_ICL <- function(x, indices) { d <- x[indices,] 
  ...
  loess_fit <- loess(ICL~var_bin, d, weights=tothh_Cpoly, control=loess.control(surface="direct"),
                     span=filter(featscales, feature==featscales[i,"feature"])[[paste0("span_",ioucty,"_ICL")]])
  predict(loess_fit, data.frame(var_bin=sort(unique(x$var_bin))), se=T)$fit}

tic()
bootres_ICL <- boot(data=df_n, statistic=loessbt_ICL, R=10)
toc()

  #####

cidf <- data.frame(var_bin = sort(unique(df_n$var_bin))) %>% mutate(county=cty, IOU=iou) %>%
...
cidf <- cidf %>%
  mutate(across(-c(var_bin, IOU, county), as.character)) %>%
  melt(id=c("var_bin", "IOU", "county")) %>%
  mutate(variable=as.character(variable),
  ...
  dcast(county+IOU+var_bin~ci, value.var="value")
  #dplyr::rename(lfit=value)

  df_ft_prior <- rbind(df_ft_prior, cidf)
}

df_ft <- df_ft %>% merge(df_ft_prior, by=c("county","IOU","var_bin"), all.x=TRUE)

write.csv(df_ft, file=paste0("figures/county/lowessci/all/",featscales[i,"f_cat"],featscales[i,"feature"],"_lfit_all.csv"), row.names=FALSE) #CHANGED HERE
}
rm(ci,cidf)
toc()
```  
...
df_p <- read.csv(paste0("figures/county/lowessci/all/", featscales[i, "f_cat"], featscales[i, "feature"], "_lfit_all.csv"),
                 header = TRUE,
                 na.strings = "?",
                 colClasses = "character")

df_p[-c(1,2)] <- lapply(df_p[-c(1,2)], function(x) as.numeric(as.character(x)))

df_ps = data.frame()
for (ioucty in iou_counties_plot) {

  iou = sapply(strsplit(ioucty, "_"), function(x) x[1])
  cty = sapply(strsplit(ioucty, "_"), function(x) x[2])
  df_pc <- df_p %>% filter(IOU == iou, county == cty)
  df_ps <- rbind(df_ps, df_pc)
}
df_ps$IOU_county <- paste(df_ps$IOU, df_ps$county, sep = "_")
df_ps <- df_ps %>%
  mutate(IOU_county = recode(factor(IOU_county, levels=iou_counties_plot),
                  'PGE_Alameda' = "Alameda", 'PGE_Santa Clara' = "Santa Clara", 'PGE_San Francisco' = "San Francisco", 'PGE_Contra Costa' = "Contra Costa", 
                  'PGE_Fresno' = "Fresno", 'PGE_San Mateo' = "San Mateo", 'PGE_San Joaquin' = "San Joaquin", 'PGE_Kern' = "Kern [PG&E]",
                  'SCE_Los Angeles' = "Los Angeles", 'SCE_Orange' = "Orange", 'SCE_San Bernardino' = "San Bernardino", 'SCE_Riverside' = "Riverside",
                  'SCE_Ventura' = "Ventura", 'SCE_Tulare' = "Tulare", 'SCE_Santa Barbara' = "Santa Barbara", 'SCE_Kern' = "Kern [SCE]"))
         
    ggplot(df_ps, aes(x=var_bin/featscales[i,"f_div"], y=wtdmed, color=tothh_Cpoly/1000, weight=tothh_Cpoly)) + 
      theme_light() + theme(legend.position="bottom") + facet_wrap(.~IOU_county, scales="free", nrow=2) +
    geom_ribbon(data=df_ps, aes(x=var_bin/featscales[i,"f_div"], ymin=ci90l, ymax=ci90h), fill="lightsteelblue1", alpha=0.5, inherit.aes=FALSE) +
    geom_ribbon(data=df_ps, aes(x=var_bin/featscales[i,"f_div"], ymin=ci50l, ymax=ci50h), fill="lightsteelblue2", alpha=0.5, inherit.aes=FALSE) +
  ...
  ggsave(paste0(featscales[i,"f_cat"],featscales[i,"feature"],"_all.png"), path="figures/county/lowesscomb/all/", width=16, height=6.5)
}
```
...
```{r}
df_p <- read.csv(paste0("figures/county/lowessci/all/",featscales[10,"f_cat"],featscales[10,"feature"],"_lfit_all.csv"), 
                 header=T, na.strings="?", colClasses=c("character")) %>% mutate(feature="racediversity")

for (i in 11:14){
df_p <- rbind(df_p, 
              mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[i,"f_cat"],featscales[i,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
                           feature=featscales[i,"feature"]))
}

df_p[-c(1, 2, ncol(df_p))] <- lapply(df_p[-c(1, 2, ncol(df_p))], function(x) as.numeric(as.character(x)))
...
plotops <- list(theme_light(), facet_wrap(.~IOU_county, scales="fixed", nrow = 2),
...
a <- ggplot(filter(df_ps), aes(x=var_bin*100, weight=tothh_Cpoly)) + plotops +
   scale_y_continuous(breaks=seq(0,4,1)) + coord_cartesian(ylim=c(0,4))

ggsave("demo_race_allcounty.png", a, path="figures/county/lowesscomb/all/", width=20, height=6.5)
...
```{r}
df_p <- rbind(mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[15,"f_cat"],featscales[15,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
                           feature=featscales[15,"feature"]), 
              mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[16,"f_cat"],featscales[16,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
...
b1 <- ggplot(filter(df_ps), aes(x=var_bin*100, weight=tothh_Cpoly)) + plotops + 
  scale_y_continuous(breaks=seq(0,4,1)) + coord_cartesian(ylim=c(0,4))

print(b1)
ggsave("demo_inc_allcounty.png", b1, path="figures/county/lowesscomb/all/", width=20, height=6.5)


df_p <- mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[17,"f_cat"],featscales[17,"feature"],"_lfit_all.csv"),
                              header=T, na.strings="?", colClasses=c("character")),
...
b2 <- ggplot(filter(df_ps), aes(x=var_bin/1000, weight=tothh_Cpoly)) + plotops +
  scale_y_continuous(breaks=seq(0,4,1)) + coord_cartesian(ylim=c(0,4))

print(b2)

ggsave("demo_inc_medhhinc_2_allcounty.png", b2, path="figures/county/lowesscomb/all/", width=20, height=6.5)
```

Lowess plots - CalEnviroScreen

```{r}
df_p <- rbind(mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[25,"f_cat"],featscales[25,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
                           feature=featscales[25,"feature"]), 
              mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[26,"f_cat"],featscales[26,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
...
c1 <- ggplot(filter(df_ps), aes(x=var_bin, weight=tothh_Cpoly)) + plotops #+ 
  #scale_y_continuous(breaks=seq(0,4,1)) + coord_cartesian(ylim=c(0,4))

print(c1)

ggsave("demo_ces_pollution_allcounty.png", c1, path="figures/county/lowesscomb/all/", width=20, height=6.5)


df_p <- rbind(mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[27,"f_cat"],featscales[27,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
                           feature=featscales[27,"feature"]),
              mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[28,"f_cat"],featscales[28,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
...
c2 <- ggplot(filter(df_ps), aes(x=var_bin, weight=tothh_Cpoly)) + plotops #+ 
  #scale_y_continuous(breaks=seq(0,4,1)) + coord_cartesian(ylim=c(0,4))

print(c2)

ggsave("demo_ces_population_allcounty.png", c2, path="figures/county/lowesscomb/all/", width=20, height=6.5)
```

Lowess plots - housing types

```{r}
df_p <- rbind(mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[18,"f_cat"],featscales[18,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
                           feature=featscales[18,"feature"]), 
              mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[19,"f_cat"],featscales[19,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
...
d1 <- ggplot(filter(df_ps), aes(x=var_bin*100, weight=tothh_Cpoly)) + plotops #+ 
  #scale_y_continuous(breaks=seq(0,4,1)) + coord_cartesian(ylim=c(0,4))

print(d1)
ggsave("demo_hs_allcounty.png", d1, path="figures/county/lowesscomb/all/", width=20, height=6.5)

df_p <- mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[21,"f_cat"],featscales[21,"feature"],"_lfit_all.csv"), 
                              header=T, na.strings="?", colClasses=c("character")),
...
d2 <- ggplot(filter(df_ps), aes(x=var_bin, weight=tothh_Cpoly)) + plotops #+ 
  #scale_y_continuous(breaks=seq(0,4,1)) + coord_cartesian(ylim=c(0,4))

print(d2)
ggsave("demo_hs_medyrbuilt_2_allcounty.png", d2, path="figures/county/lowesscomb/all/", width=20, height=6.5)
```
...
ggsave("figures/county/lowesscomb/all/demo_cesinchs_allcounty.png", width=30, height=10)
# ggsave("figures/county/lowesscomb/demo_cesinchs.pdf", width=16, height=20)

```


Lowess plots - housing types (all)

```{r}
df_p <- read.csv(paste0("figures/county/lowessci/all/",featscales[19,"f_cat"],featscales[19,"feature"],"_lfit_all.csv"),
                 header=T, na.strings="?", colClasses=c("character")) %>% mutate(feature=featscales[19,"feature"])

for (i in 29:32){
df_p <- rbind(df_p, mutate(read.csv(paste0("figures/county/lowessci/all/",featscales[i,"f_cat"],featscales[i,"feature"],"_lfit_all.csv"), 
                                    header=T, na.strings="?", colClasses=c("character")),
                           feature=featscales[i,"feature"]))
}
...
plotops <- list(theme_light(), facet_wrap(.~IOU_county, scales="fixed", nrow = 2),
  ...
  scale_x_continuous(breaks=seq(0,100,by=20)), #scale_y_continuous(breaks=scales::pretty_breaks(3)),
  scale_color_discrete_qualitative(palette="Dark3"), scale_fill_discrete_qualitative(palette="Dark3"),
  labs(x="Type of housing (%)", y=plotlabs[1], fill="Feature", color="Feature"))

e <- ggplot(filter(df_ps), aes(x=var_bin*100, weight=tothh_Cpoly)) + plotops +
   scale_y_continuous(breaks=seq(0,4,1)) + coord_cartesian(ylim=c(0,4))

ggsave("demo_housing_types_allcounty.png", e, path="figures/county/lowesscomb/all/", width=20, height=6.5)

print(e)
```

Lowess plots - housing types (3 types: Multi, Large-Multi, Small-Multi)

```{r}
df_p <- read.csv(paste0("figures/county/lowessci/all/",featscales[19,"f_cat"],featscales[19,"feature"],"_lfit_all.csv"),
                 header=T, na.strings="?", colClasses=c("character")) %>% mutate(feature=featscales[19,"feature"])
df_p <- rbind(df_p, read.csv(paste0("figures/county/lowessci/all/",featscales[29,"f_cat"],featscales[29,"feature"],"_lfit_all.csv"),
...
df_ps <- df_ps %>%
  mutate(feature = recode(factor(feature, levels=c("singleunit_pct","smallmulti_pct","largemulti_pct","MUD_pct")),
                          'singleunit_pct'="Single Unit", 'smallmulti_pct'="Small MUD", 'largemulti_pct'="Large MUD", 'MUD_pct'="All MUD (including duplex)"))
...
plotops <- list(theme_light(), facet_wrap(.~IOU_county, scales="fixed", nrow = 2),
  ...
  labs(x="Type of housing (%)", y=plotlabs[1], fill="Type of Housing", color="Type of Housing"))

e <- ggplot(filter(df_ps), aes(x=var_bin*100, weight=tothh_Cpoly)) + plotops +
   scale_y_continuous(breaks=seq(0,10,1)) + coord_cartesian(ylim=c(0,10))

ggsave("demo_MUDtypes_allcounty.png", e, path="figures/county/lowesscomb/all/", width=20, height=6.5)
